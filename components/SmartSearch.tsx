...
const SmartSearch: React.FC<SmartSearchProps> = ({ report }) => {
  ...
  const handleSearch = useCallback(async (e: React.FormEvent) => {
    ...
    logger.log('SmartSearch', 'INFO', `Iniciando busca inteligente para a query: "${query}"`, undefined, {
      correlationId,
      scope: 'llm',
    });
    ...
    try {
      ...
      logger.log('SmartSearch', 'INFO', 'Busca inteligente concluída com sucesso.', undefined, {
        correlationId,
        scope: 'llm',
      });
    } catch (err) {
      ...
      logger.log('SmartSearch', 'ERROR', 'Falha na busca inteligente.', { error: err }, {
        correlationId: telemetry.createCorrelationId('llm'),
        scope: 'llm',
      });
    } finally {
      setIsLoading(false);
                  ...
                  {tableRows.map((row, rowIndex) => (
                    <tr key={rowIndex} className="hover:bg-gray-600/20">
                      {row.map((cell, cellIndex) => (
                        <td
                          key={tableHeaders[cellIndex] || cellIndex}
                          className="px-4 py-2 text-gray-300 font-mono"
                        >
                          {cell}
                        </td>
                      ))}
